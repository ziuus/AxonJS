...
`;
  ...
  async runStandardProvider(messages, context) {
    ...
    switch (this.config.llmProvider) {
      case "openai":
        providerFunc = createOpenAI({ apiKey: this.config.apiKey });
        break;
      case "gemini":
        providerFunc = createGoogleGenerativeAI({ apiKey: this.config.apiKey });
        break;
      case "anthropic":
        providerFunc = createAnthropic({ apiKey: this.config.apiKey });
        break;
      case "mistral":
        providerFunc = createMistral({ apiKey: this.config.apiKey });
        break;
      case "perplexity":
        providerFunc = createPerplexity({ apiKey: this.config.apiKey });
        break;
      case "cohere":
        providerFunc = createCohere({ apiKey: this.config.apiKey });
        break;
      default:
        providerFunc = createOpenAI({ apiKey: this.config.apiKey });
    }
  ...
  tools: [
    {
      ...
      schema: z5.object({
        gesture: z5.enum(["idle", "talking", "thinking", "success", "failure", "wave", "nod", "shakehead", "rotate", "spin", "walk", "dance", "agree", "disagree"]).describe("The physical gesture or state to perform")
      }),
    ...
    },
    {
      ...
      schema: z5.object({
        expression: z5.enum(["neutral", "happy", "thinking", "concerned", "surprised", "smiling"]).describe("The facial expression to apply")
      }),
    ...
    },
    {
      ...
      schema: z5.object({
        posture: z5.enum(["standing", "sitting", "attentive", "relaxed"]).describe("The base posture state")
      }),
    ...
    },
    {
      ...
      schema: z5.object({
        target: z5.string().describe('The name of the object to look at, or "user"'),
        intensity: z5.number().min(0).max(1).optional().describe("How much the character should turn (0-1)")
      }),
